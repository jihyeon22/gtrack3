...
static int _server_ip_set_dtg(int argc, char **argv);
static int _server_ip_set_dvr(int argc, char **argv);
...
int parse_model_sms(char *time, char *phonenum, char *sms)
{
	...
	switch(sms_cmd) {
		...
		case eSMS_GEO_FENCE_SET:
			...
			break;
		case eSMS_IP_SETUP_DTG:
			ret = _server_ip_set_dtg(model_argc, model_argv);
			break;
		case eSMS_IP_SETUP_DVR:
			ret = _server_ip_set_dvr(model_argc, model_argv);
			break;
...
static int _server_ip_set_dtg(int argc, char **argv)
{
	...
	strncpy(conf->model.dtg_report_ip, p_str_ip, sizeof(conf->model.dtg_report_ip));
	conf->model.dtg_report_port = atoi(p_str_port);

	if(save_config_user("dtg:report_ip", p_str_ip) < 0)
	{
		LOGE(LOG_TARGET, "<%s> save config error #1\n", __FUNCTION__);
		return -1;
	}
	if(save_config_user("dtg:report_port", p_str_port) < 0)
	{
		LOGE(LOG_TARGET, "<%s> save config error #2\n", __FUNCTION__);
		return -1;
	}
	...
	{
	//	char smsmsg[100] = {0,};
   	//	sender_add_data_to_buffer(eSMS_IP_SETUP_DTG, NULL, ePIPE_2);
...
static int _server_ip_set_dvr(int argc, char **argv)
{
	...
	strncpy(conf->model.dvr_report_ip, p_str_ip, sizeof(conf->model.dvr_report_ip));
	conf->model.dvr_report_port = atoi(p_str_port);

	if(save_config_user("dvr:report_ip", p_str_ip) < 0)
	{
		LOGE(LOG_TARGET, "<%s> save config error #1\n", __FUNCTION__);
		return -1;
	}
	if(save_config_user("dvr:report_port", p_str_port) < 0)
	{
		LOGE(LOG_TARGET, "<%s> save config error #2\n", __FUNCTION__);
		return -1;
	}
	...
	{
		char smsmsg[100] = {0,};
   	//	sender_add_data_to_buffer(eSMS_IP_SETUP_DVR, NULL, ePIPE_2);
