...
configurationModel_t* load_config_user(void)
{
	...
	while(++ntry <= INI_RETRY_CNT)
	{
        ...
        // dtg data
		if(get_value_ini_str(inf, "dtg:report_ip", config.model.dtg_report_ip, sizeof(config.model.dtg_report_ip)) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "dtg:report_port", &config.model.dtg_report_port) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "dtg:collect_interval_keyon", &config.model.dtg_collect_interval_keyon) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "dtg:collect_interval_keyoff", &config.model.dtg_collect_interval_keyoff) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "dtg:report_interval_keyon", &config.model.dtg_report_interval_keyon) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "dtg:report_interval_keyoff", &config.model.dtg_report_interval_keyoff) < 0)
		{
			continue;
		}

        // dvr data
		if(get_value_ini_str(inf, "dvr:report_ip", config.model.dvr_report_ip, sizeof(config.model.dvr_report_ip)) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "dvr:report_port", &config.model.dvr_report_port) < 0)
		{
			continue;
		}
...
//	conf_base->mileage.enable = 0;
	strcpy(conf_base->common.sms_passwd, "bic");
}

void load_config_model_default(void)
{
	config.model.tempature_enable = 0;
	strcpy(config.model.tempature_device, "/dev/ttyHSL1");
...
{
	strcpy(config.model.report_ip, "13.124.141.97");
	config.model.report_port = 7000;
    ...
    strcpy(config.model.dtg_report_ip, "13.124.241.79");
    config.model.dtg_report_port = 7000;
    config.model.dtg_collect_interval_keyon = 1;
    config.model.dtg_collect_interval_keyoff = 180;
    config.model.dtg_report_interval_keyon = 1;
    config.model.dtg_report_interval_keyoff = 60;

    strcpy(config.model.dvr_report_ip, "13.124.246.47");
    config.model.dvr_report_port = 7000;

	strcpy(config.model.model_name, "bicd");
