...
RETRY_SEND:
    ...
    switch (op)
    {
        ...
        case e_bcm_knocksensor_setting_val: // 0x51 : 노크센서 설정 정보
        {
            ALLOC_PKT_RECV__KNOCK_SENSOR_INFO_REQ recv_buff;

            recv_buff_len = sizeof(recv_buff);
            memset(&recv_buff, 0x00, recv_buff_len);

            LOGI(eSVC_MODEL, "[ALLOC2 NETCOMM] send pkt [0x%x] - e_bcm_knocksensor_setting_val \r\n", e_firm_info);
            ...
            if (recv_ret == 0)
            {
                ...
                //mds_api_debug_hexdump_buff(&recv_buff, recv_buff_len);
                //printf("-------------------------------------------------\r\n");
                LOGI(eSVC_MODEL, "[ALLOC2 PKT TRANS] e_bcm_knocksensor_setting_val >> SUCCESS evtcode [0x%x] recv_ret is [%d] \r\n", op, recv_ret);

                send_pkt_ret = parse_pkt__bcm_knocksensor_setting_val(&recv_buff);
            }
            else
            {
                LOGE(eSVC_MODEL, "[ALLOC2 PKT TRANS] e_bcm_knocksensor_setting_val >> ERROR evtcode [0x%x] recv_ret is [%d] \r\n", op, recv_ret);
                send_pkt_ret = -1;
            }
            break;
        }
