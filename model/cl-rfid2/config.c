...
configurationModel_t* load_config_user(void)
{
	...
	while(++ntry <= INI_RETRY_CNT)
	{
		...
		if(get_value_ini_str(inf, "user:request_rfid_api_0", config.model.request_rfid_api_0, sizeof(config.model.request_rfid_api_0)) < 0)
		{
			continue;
		}
		if(get_value_ini_str(inf, "user:request_rfid_api_1", config.model.request_rfid_api_1, sizeof(config.model.request_rfid_api_1)) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "user:request_rfid_type", &config.model.request_rfid_type) < 0)
		{
			continue;
		}
