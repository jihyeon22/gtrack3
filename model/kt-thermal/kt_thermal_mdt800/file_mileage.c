...
void _print_mileage_status(lotte_mileage_t data)
{
	...
	FILE *fp = NULL;
	FILE *log_fd = NULL;
	remove("/var/log/mileage.log");
	fp = fopen("/var/log/mileage.log", "w");
	...
	fprintf(log_fd, "device mileage : [%d]\n", data.mileage);
...
int load_mileage_file(int *file_mileage)
{
	...
	ret = storage_load_file(MILEAGE_FILE, &data, sizeof(lotte_mileage_t));
	LOGD(LOG_TARGET, "load_mileage_file = [%d]\n", ret);
	if(ret != ERR_NONE)
	{
		_default_mileage_data(&data);
		ret = storage_save_file(MILEAGE_FILE, &data, sizeof(lotte_mileage_t));
