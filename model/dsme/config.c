...
configurationModel_t* load_config_user(void)
{
	...
	while(++ntry <= INI_RETRY_CNT)
	{
		...
		if(get_value_ini_str(inf, "user:control_ip", config.model.control_ip, sizeof(config.model.control_ip)) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "user:control_port", &config.model.control_port) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "user:report_interval", &config.model.report_interval) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "user:board_rate", &config.model.board_rate) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "user:InV_T_collection_interval", &config.model.InV_T_collection_interval) < 0)
		{
			continue;
		}
...
void load_config_base_default(void)
{
	...
	strcpy(conf_base->common.sms_passwd, "dsme");
...
{
	strncpy(config.model.report_ip, "152.149.8.161", sizeof("152.149.8.161"));
	config.model.report_port = 58731;
	
	strncpy(config.model.control_ip, "152.149.8.161", sizeof("152.149.8.161"));
	config.model.control_port = 58731;

	config.model.report_interval = 60;
	config.model.board_rate = 38400;
	config.model.InV_T_collection_interval = 30;

	strncpy(config.model.model_name, "dsme", sizeof("dsme"));
