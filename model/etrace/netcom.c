		...
		else if(conf->model.packet_type == 65)
		{
			...
			if(packet_count != list_count-1)
			{
				...
				else
				{
					...
					gps_time.tm_year   = p_packet->date.year - 1900;   // 주의 :년도는 1900년부터 시작
					gps_time.tm_mon    = p_packet->date.mon - 1;      // 주의 :월은 0부터 시작
					...
					gps_time.tm_isdst  = 0;           // 썸머 타임 사용 안함
		...
		else
		{
			...
			if(packet_count != list_count-1)
			{
				...
				else
				{
					...
					gps_time.tm_year   = p_packet2->date.year - 1900;   // 주의 :년도는 1900년부터 시작
					gps_time.tm_mon    = p_packet2->date.mon - 1;      // 주의 :월은 0부터 시작
