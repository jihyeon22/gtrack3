...
configurationModel_t* load_config_user(void)
{
	...
	while(++ntry <= INI_RETRY_CNT)
	{
		...
		if(get_value_ini_str(inf, "user:request_http_ip", config.model.request_http_ip, sizeof(config.model.request_http_ip)) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "user:request_http_port", &config.model.request_http_port) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "user:http_send_interval_keyon", &config.model.http_send_interval_keyon) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "user:http_send_interval_keyoff", &config.model.http_send_interval_keyoff) < 0)
		{
			continue;
		}
		if(get_value_ini_int(inf, "user:thermal_sensor_interval", &config.model.thermal_sensor_interval) < 0)
		{
			continue;
		}
		if(get_value_ini_str(inf, "user:thermal_sensor_dev", config.model.thermal_sensor_dev, sizeof(config.model.thermal_sensor_dev)) < 0)
		{
			continue;
		}
...
{
	strncpy(config.model.report_ip, "null.co.kr", sizeof("null.co.kr"));
	config.model.report_port = 1111;
	strncpy(config.model.request_http_ip, "1.229.253.27", sizeof("1.229.253.27"));
	config.model.request_http_port = 8090;
	config.model.http_send_interval_keyon = 60;
	config.model.http_send_interval_keyoff = 60;
	config.model.thermal_sensor_interval = 30;
	strncpy(config.model.thermal_sensor_dev, "/dev/ttyHSL2", sizeof("/dev/ttyHSL2"));
	
	strncpy(config.model.model_name, "ds-thermal", sizeof("ds-thermal"));
}
