...
static int _enable_log(int argc, char **argv);
...
int parse_model_sms(char *time, char *phonenum, char *sms)
{
	...
	switch(sms_cmd) {
		...
		case eSMS_GET_MODEM_STATUS:
			...
			break;
#endif
		case eSMS_ENABLE_LOG:
			ret = _enable_log(model_argc, model_argv);
			break;
		default:
...
//	thread_network_set_warn_timeout(MAX(conf->model.report_interval_keyon, conf->model.report_interval_keyoff) * 2);
	...
	{
		char smsmsg[100] = {0,};
		sender_add_data_to_buffer(eREPORT_CYCLE_NUM2_SETUP_EVT, NULL, ePIPE_1);
