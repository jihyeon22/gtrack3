...
//	printf( "fence >> routine :: pos lat:%3.7f, lon:%3.7f entry\n",  pData->lat, pData->lon);
	...
	if(get_geo_fence4_enable()) {
		fence_chk = get_geofence4_notification(*pData);
		if(fence_chk == eFENCE_IN_NOTIFICATION) {
			LOGT(LOG_TARGET, "geo fence #3 entry notification!!!\n");
			sender_add_data_to_buffer(eGEO_FENCE_NUM4_ENTRY_EVT , NULL, ePIPE_2);

			sprintf(smsmsg, "fence4 pos lat:%3.7f, lon:%3.7f entry\n",  pData->lat, pData->lon);
			printf("%s> %s", __func__, smsmsg);
			devel_send_sms_noti(smsmsg, strlen(smsmsg), 3);
		}
		else if(fence_chk == eFENCE_OUT_NOTIFICATION) {
			LOGT(LOG_TARGET, "geo fence #3 exit notification!!!\n");
			sender_add_data_to_buffer(eGEO_FENCE_NUM4_EXIT_EVT  , NULL, ePIPE_2);

			sprintf(smsmsg, "fence4 pos lat:%3.7f, lon:%3.7f exit\n",  pData->lat, pData->lon);
			printf("%s> %s", __func__, smsmsg);
			devel_send_sms_noti(smsmsg, strlen(smsmsg), 3);
		}
	}
