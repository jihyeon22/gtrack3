...
geo_fence_hdr_t g_geo_fence = 
{
	...
	.fence4.enable = eGEN_FENCE_DISABLE,
};
...
void _default_geo_fence(geo_fence_hdr_t *pdata)
{
	...
	pdata->fence4.enable             = eGEN_FENCE_DISABLE;
	pdata->fence4.latitude           = 0.0;
	pdata->fence4.longitude          = 0.0;
	pdata->fence4.range              = 0;
	pdata->fence4.cur_fence_status   = eFENCE_SETUP_UNKOWN;
	pdata->fence4.setup_fence_status = eFENCE_STATUS_UNKOWN;
}
...
void _print_geo_fence_status()
{
	...
	switch(g_geo_fence.fence3.enable) {
	...
	// -----------------------------------------
	fprintf(log_fd, "[GEO FENCE #4]\n");
	switch(g_geo_fence.fence4.enable) {
		case eGEN_FENCE_DISABLE: fprintf(log_fd, "\tgeo fence4 disable\n"); break;
		case eGEN_FENCE_ENABLE: fprintf(log_fd, "\tgeo fence4 enable\n"); break;
		default: fprintf(log_fd, "\tgeo fence4 unknow enable status\n"); break;
	}
	fprintf(log_fd, "\tfence4 latitude = %06.6f\n", g_geo_fence.fence4.latitude);
	fprintf(log_fd, "\tfence4 longitude = %06.6f\n", g_geo_fence.fence4.longitude);
	fprintf(log_fd, "\tfence4 range = %d\n", g_geo_fence.fence4.range);
	
	switch(g_geo_fence.fence4.cur_fence_status) {
		case eFENCE_STATUS_UNKOWN: fprintf(log_fd, "\tgeo fence4 unkown status #1\n"); break;
		case eFENCE_STATUS_IN: fprintf(log_fd, "\tgeo fence4 IN status\n"); break;
		case eFENCE_STATUS_OUT: fprintf(log_fd, "\tgeo fence4 OUT status\n"); break;
		case eFENCE_STATUS_IGNORE: fprintf(log_fd, "\tgeo fence4 ignore status\n"); break;
		default: fprintf(log_fd, "\tgeo fence4 unknown status #2\n"); break;
	}

	switch(g_geo_fence.fence4.setup_fence_status) {
		case eFENCE_SETUP_UNKOWN: fprintf(log_fd, "\tgeo fence4 unkown setup\n"); break;
		case eFENCE_SETUP_ENTRY: fprintf(log_fd, "\tgeo fence4 entry setup\n"); break;
		case eFENCE_SETUP_EXIT: fprintf(log_fd, "\tgeo fence4 exit setup\n"); break;
		case eFENCE_SETUP_ENTRY_EXIT: fprintf(log_fd, "\tgeo fence4 entry & exit status\n"); break;
		default: fprintf(log_fd, "\tgeo fence4 unknown status #2\n"); break;
	}
...
int clear_geo_fence(int idx, int save_flag)
{
	...
	if ( (idx < 0) || (idx > MAX_FENCE_DATA_CNT) )
		return 0;
	...
	switch(idx)
	{
		case 0:
		{
			g_geo_fence.fence0.enable             = eGEN_FENCE_DISABLE;
			g_geo_fence.fence0.latitude           = 0.0;
			g_geo_fence.fence0.longitude          = 0.0;
			g_geo_fence.fence0.range              = 0;
			g_geo_fence.fence0.cur_fence_status   = eFENCE_SETUP_UNKOWN;
			g_geo_fence.fence0.setup_fence_status = eFENCE_STATUS_UNKOWN;
			break;
		}
		case 1:
		{
			g_geo_fence.fence1.enable             = eGEN_FENCE_DISABLE;
			g_geo_fence.fence1.latitude           = 0.0;
			g_geo_fence.fence1.longitude          = 0.0;
			g_geo_fence.fence1.range              = 0;
			g_geo_fence.fence1.cur_fence_status   = eFENCE_SETUP_UNKOWN;
			g_geo_fence.fence1.setup_fence_status = eFENCE_STATUS_UNKOWN;
			break;
		}
		case 2:
		{
			g_geo_fence.fence2.enable             = eGEN_FENCE_DISABLE;
			g_geo_fence.fence2.latitude           = 0.0;
			g_geo_fence.fence2.longitude          = 0.0;
			g_geo_fence.fence2.range              = 0;
			g_geo_fence.fence2.cur_fence_status   = eFENCE_SETUP_UNKOWN;
			g_geo_fence.fence2.setup_fence_status = eFENCE_STATUS_UNKOWN;
			break;
		}
		case 3:
		{
			g_geo_fence.fence3.enable             = eGEN_FENCE_DISABLE;
			g_geo_fence.fence3.latitude           = 0.0;
			g_geo_fence.fence3.longitude          = 0.0;
			g_geo_fence.fence3.range              = 0;
			g_geo_fence.fence3.cur_fence_status   = eFENCE_SETUP_UNKOWN;
			g_geo_fence.fence3.setup_fence_status = eFENCE_STATUS_UNKOWN;
			break;
		}
		case 4:
		{
			g_geo_fence.fence4.enable             = eGEN_FENCE_DISABLE;
			g_geo_fence.fence4.latitude           = 0.0;
			g_geo_fence.fence4.longitude          = 0.0;
			g_geo_fence.fence4.range              = 0;
			g_geo_fence.fence4.cur_fence_status   = eFENCE_SETUP_UNKOWN;
			g_geo_fence.fence4.setup_fence_status = eFENCE_STATUS_UNKOWN;
			break;
		}
		default:
		{
...
{
	int ret;
	memcpy(&g_geo_fence.fence4, &data, sizeof(geo_fence_data_t));
	ret = storage_save_file(GEO_FENCE_STATUS_FILE, &g_geo_fence, sizeof(geo_fence_hdr_t));
	_print_geo_fence_status();
	return ret;
}
...
fence_notification_t get_geofence4_notification(gpsData_t cur_gps)
{
	...
	if(geo_fence4_wait_count-- > 0) {
		return eFENCE_NONE_NOTIFICATION;
	}

	result = _get_notification(&g_geo_fence.fence4, cur_gps);
	if(result != eFENCE_NONE_NOTIFICATION)
	{
		switch(g_geo_fence.fence4.setup_fence_status)
		{
		...
		geo_fence4_wait_count = WAIT_TIME_UNTIL_NEXT_GEO_EVENT;
		set_geo_fence4(g_geo_fence.fence4);
	}
...
{
	return g_geo_fence.fence4.enable;
}
...
int get_geo_first_event()
{
	int evt_flag = 0;

	if ( mds_api_check_exist_file(GEO_FENCE_EVENT_FLAG, 1) == DEFINES_MDS_API_OK )
		evt_flag = 1;
	else 
		evt_flag = 0;

	// remove(GEO_FENCE_EVENT_FLAG);

	return evt_flag;
}

int set_geo_event_flag(int flag)
{
	char event_flag_touch_cmd[128] = {0,};

	sprintf(event_flag_touch_cmd, "/bin/touch %s &", GEO_FENCE_EVENT_FLAG);
	
	if ( flag )
		system(event_flag_touch_cmd);
	else
		remove(GEO_FENCE_EVENT_FLAG);
