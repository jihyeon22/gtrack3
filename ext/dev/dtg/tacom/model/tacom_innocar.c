...
//printf("\n\n");
		...
		do{
			...
			{
				DTG_LOGE("Header Request Can't Ack from DTG #1 ret[%d/%d] retry[%d]", total_read_bytes, sizeof(tacom_inno_hdr_t), read_cnt);
				sleep(3);
				continue;
...
int data_convert(tacom_std_data_t *std_data, unsigned char *dtg_data, int debug_flag) {
	...
	//x : longitude(�浵), 127.xxx
	//y : latitude(����),  36.xxx
	if(debug_flag == 0)
		DTG_LOGT("DTG ORIGINAL GPS x, y = %ld, %ld", inno_data->longitude, inno_data->latitude);
	...
	longitude = gps_chnage(inno_data->longitude);
	sprintf(std_data->gps_x, "%09ld", longitude); 
	latitude = gps_chnage(inno_data->latitude);  
	sprintf(std_data->gps_y, "%09ld",latitude); 

	if(debug_flag == 0)
		DTG_LOGT("DTG CONVERT GPS x, y = %ld, %ld", longitude, latitude);
