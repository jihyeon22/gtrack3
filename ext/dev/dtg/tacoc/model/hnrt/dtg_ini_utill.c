...
int save_ini_dtg_period_info()
{
	...
	iniparser_set(g_ini_handle, "dtg_config:create_period", tmp);
...
int save_ini_maintain_info()
{
	...
	iniparser_set(g_ini_handle, "maintain_info:trip_number", tmp);
	sprintf(tmp, "%d", get_cumulative_dist());
	iniparser_set(g_ini_handle, "maintain_info:cumulative_distance", tmp);
...
int load_ini_file()
{
	...
	num = iniparser_getint(g_ini_handle, "dtg_config:create_period", -1);
	if(num == -1) return -1;
	...
	num = iniparser_getint(g_ini_handle, "maintain_info:trip_number", -1);
	if(num == -1) return -1;
	set_trip_number(num);

	num = iniparser_getint(g_ini_handle, "maintain_info:cumulative_distance", -1);
	if(num == -1) return -1;
