...
#if defined(DSERVER_MODEL_HNRT)
#if defined(BOARD_TL500S) || defined(BOARD_TL500K)
	memset(dtg_vrn, 0, sizeof(dtg_vrn));
	if(get_vrn_info(tmp, DTG_VRN_SIZE) >= 0)
		strncpy(dtg_vrn, tmp, DTG_VRN_SIZE);
	else
...
void send_server_error_report(char *packet_type, int server_resp)
{
	...
	//sprintf(log, "packet : %s, vrn : %s, ip/port : %s/%d, server_resp %d", packet_type, tmp, get_server_ip_addr(), get_server_port(), server_resp);
