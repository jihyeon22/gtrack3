...
int load_ini_file()
{
	...
	num = iniparser_getint(g_dtg_ini_handle, "dtg_config:volt_report_key_on_period", -1);
	if(num == -1) return -1;
	set_volt_key_on_report_period(num);
	
	num = iniparser_getint(g_dtg_ini_handle, "dtg_config:volt_report_key_off_period", -1);
	if(num == -1) return -1;
