...
ifeq ($(USE_GPS_DEACTIVE_RESET),y)
CFLAGS  += -DMDS_FEATURE_USE_GPS_DEACTIVE_CORRECT
endif
...
else ifeq ($(USE_KJTEC_RFID),y)
	$(Q)fakeroot cp -v $(MODEL_PATH)/ext/rfid/kjtec_rfid/rfid_fw/rfid_fw_default/rfid_fw*.bin $(DESTDIR)$(WORK_PATH)/
endif
endif
