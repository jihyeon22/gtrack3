...
void _init_geo_fence_data()
{
	...
	{
		g_setup_data[i].enable = eGEN_FENCE_V2_DISABLE;
		g_setup_data[i].latitude = 0.0;
		g_setup_data[i].longitude = 0.0;
		g_setup_data[i].range = 0;
		g_setup_data[i].setup_fence_status = eFENCE_V2_SETUP_UNKNOWN;
...
fence_v2_notification_t _check_fence_data(int dist, int entry_range, int exit_range)
{
	...
	if (dist > exit_range)
		return eFENCE_V2_OUT_NOTIFICATION;
		//fence out
...
char * _print_geo_fence_notification(fence_v2_notification_t check_status)
{
	if(check_status == eFENCE_V2_IN_NOTIFICATION)
		return "eFENCE_V2_IN_NOTIFICATION";
	else if(check_status == eFENCE_V2_OUT_NOTIFICATION)
		return "eFENCE_V2_OUT_NOTIFICATION";
...
*/
	...
	{
		case eFENCE_V2_SETUP_ENTRY_EXIT:
		case eFENCE_V2_SETUP_ENTRY:
		case eFENCE_V2_SETUP_EXIT:
			break;
		default:
			result = eFENCE_V2_NONE_NOTIFICATION;
			break;
	}
...
*/
	if(result != eFENCE_V2_NONE_NOTIFICATION)
	{
		if(result != g_status_data[idx].cur_fence_status)
		{
			...
			if(g_setup_data[idx].setup_fence_status == eFENCE_V2_SETUP_ENTRY)
			{
				if(result != eFENCE_V2_IN_NOTIFICATION)
					result = eFENCE_V2_NONE_NOTIFICATION;
			}
			else if(g_setup_data[idx].setup_fence_status == eFENCE_V2_SETUP_EXIT)
			{
				if(result != eFENCE_V2_OUT_NOTIFICATION)
					result = eFENCE_V2_NONE_NOTIFICATION;
			}
			return result;
		}
...
{
    save_geo_fence_status_info_v2();
}

int set_geo_fence_setup_info_v2(int idx, geo_fence_v2_setup_t *data)
{
	if(idx >= GEO_FENCE_V2_MAX_COUNT || idx < 0)
		return -1;

	memcpy(&g_setup_data[idx], data, sizeof(geo_fence_v2_setup_t));
...
int get_geo_fence_setup_info_v2(int idx, geo_fence_v2_setup_t *data)
{
	...
	memcpy(data, &g_setup_data[idx], sizeof(geo_fence_v2_setup_t));
