...
int mileage_process_2(const gpsData_t *temp_gpsdata)
{
            ...
            md->lastlon < 123.263168 || md->lastlon > 132.074203)
    {
        ...
        md->lastlat = temp_gpsdata->lat;
        md->lastlon = temp_gpsdata->lon;
        last_active_stat = temp_gpsdata->active;
        return 0;
    }

    cur_active_stat = temp_gpsdata->active;

    if (temp_gpsdata->speed <= 0)
        need_to_calc = 0;
    
    // gps stat change is force calc..
    ...
    if ( need_to_calc == 0 )
    {
        ...
        LOGE(LOG_TARGET, "calc nok... last [%d] cur [%d], speed [%d] \r\n",last_active_stat, cur_active_stat, temp_gpsdata->speed);
        return 0;
    }
    
    LOGT(LOG_TARGET, "calc ok... last [%d] cur [%d], speed [%d]  \r\n",last_active_stat, cur_active_stat, temp_gpsdata->speed);

    current_time = tools_get_kerneltime();

    // always calc..
    if ( ( force_calc == 1 ) || (( current_time - last_mileage_time > 0 ) && ( cur_active_stat == 1 ) ))
    {	
        last_mileage_time = current_time;
