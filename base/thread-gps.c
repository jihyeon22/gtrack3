...
void *thread_gps(void *args)
{
	...
	while(flag_run_thread_gps)
	{
		...
		// check gps data 1
		if ( udp_gpsd_err_cnt1 > udp_gpsd_err_chk_cnt1 )
		{
			LOGE(LOG_TARGET, "get gps data fail 1 : cannot recv form gpsd , gpsd reset\n");
			devel_webdm_send_log("GPSD req reset 1 : cannot recv gps data [%d]", udp_gpsd_err_cnt2);

			gps_reset(GPS_BOOT_WARM);
			udp_gpsd_err_cnt1 = 0;
			udp_gpsd_err_cnt2++;

			udp_gpsd_err_chk_cnt1 = MAX_UDP_GPSD_MAX_ERR_1_CNT;
		}
		...
		if ( udp_gpsd_err_cnt2 > MAX_UDP_GPSD_MAX_ERR_2_CNT )
		{
			...
			// 여기까지 왔으면, mds_gpsd3 가 이상한것이다. 때문에 아예 kill 을 시켜서 다시 시작하도록한다.
			system("killall mds_gpsd3 &");

			udp_gpsd_err_cnt1 = 0;
			udp_gpsd_err_cnt2 = 0;

			// gpsd3 를 kill 한다음에는 mon에서 다시 띄울때까지는 기다려야한다.
			// 때문에 60초를 쉬어주도록 한다.
			udp_gpsd_err_chk_cnt1 = 60;
		}
